...
def main():
    ...
    for i in range(0, len(dev_frame['text'])):
        tokenized_text_dev.append(tokenize(dev_frame['text'][i]))
    X_dev = feat_extractor.transform_list(tokenized_text_dev)
    ...
    print("Time for training and test (and dev): %.2f seconds" % (time.time() - start_time))
