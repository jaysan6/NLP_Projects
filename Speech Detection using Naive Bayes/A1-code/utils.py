...
class UnigramFeature(FeatureExtractor):
    ...
    def fit(self, text_set: list):
        ...
        # code to get the words with highest and lowest P(w|1) / P(w|0) ratios
        # key_list = list(self.unigram.keys())
        # val_list = list(self.unigram.values())
        # #ind = [ 701,2138,4840 ,591 ,640,1117,2017,1346,519,478]
        # #ind = [ 240,398,1267,2346 , 125 , 606,  328, 1862, 1247, 1593]
        # for i in ind:
...
class BigramFeature(FeatureExtractor):
    ...
    def fit(self, text_set):
        ...
        """
        index = 0
        for i in range(0, len(text_set),1):
            for j in range(0,len(text_set[i]))
                bi = text_set[i][j].lower() + text_set[i][j + 1].lower()
                print(bi)
        """
