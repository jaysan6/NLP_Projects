...
Original file is located at
    https://colab.research.google.com/drive/1vCcGtWsILY8W6Cx_BPOi48U4N0poDzYM
"""
...
def decode(input_length, tagset, score):
    ...
    for k in tagset:
      viterbi[(1,k)] = score(k, "<START>", 1)  # INITIALIZATION

    for m in range(2, M+1): # RECURSIVE
      for k in tagset:
        
        best_local_score = float('-inf')
        best_tag = None
        for kk in tagset:
          local_score = score(k, kk, m) + viterbi[(m-1, kk)]
          if local_score >= best_local_score:
            best_local_score = local_score
            best_tag = kk
        
        viterbi[(m, k)] = best_local_score
        bp[(m-1, k)] = best_tag     

    end_tag = None
    best_local_end_score = float('-inf')  # ENDING TAG
    for k in tagset:
      local_score = score("<STOP>", k, M)
      if local_score >= best_local_end_score:
        best_local_end_score = local_score
        end_tag = k

    y[M] = end_tag
    for m in reversed(range(1, M)):  # BACKPOINTER
     y[m] = bp[(m, y[m+1])]
